...
Everything code-side is done and verified: 15 on-chain tests pass, the program
builds, the site is live at bullbank.win, the RPC key can no longer leak, and
the crank's dead Jupiter endpoint is fixed.

Three wallets exist:

| | address |
|---|---|
| cold (upgrade authority — keep offline) | `51hkha4mobkQ4rdM5Rhuj576C17bu9FETGEG4NgprgGP` |
| crank (creator + buyback — goes in GitHub secrets) | `AUL6SSKPV9xMD81RTkhTkXw4SSYYxE48qfZK2PZGgAmM` |

The program keeps its existing identity `BLj7FScr8f57ygqFGJJtZ3sVRCx7C7gUQihyUWYptBYq`
(matches `declare_id!`, keypair at `target/deploy/memecoin_staking-keypair.json`,
already deployed to devnet as public proof of testing). A third keypair was
generated by mistake and is not used.

**Next action: fund the cold wallet with ~3.6 SOL, then deploy.**

```
solana program deploy   --url mainnet-beta   --keypair ~/bullbank-cold.json   --upgrade-authority ~/bullbank-cold.json   --program-id ~/bullbank-staking/target/deploy/memecoin_staking-keypair.json   --max-len 500000   ~/bullbank-staking/target/deploy/memecoin_staking.so
```

Deploy rent measured on mainnet: 2.62 SOL at no headroom, **3.48 SOL at
`--max-len 500000`** (leaves room to ship a fix), 5.23 SOL at the default.
The token on ansem.io is ~1 SOL more, from the crank wallet.

Still undecided: the emission rate. Permanent once `initialize_pool` runs.
